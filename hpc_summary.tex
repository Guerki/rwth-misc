...
\begin{description}[style=nextline]
	\item[What is a hot spot?]
        A part of the code where a dominant fraction of the execution 
        time is spent.

	\item[What kind of trigger mechanisms do exist?]
        Event-driven and trigger-driven.

	\begin{description}[style=nextline]
		\item[What is the difference between event-driven and sample-driven triggers?]
            \textbf{Event}-driven triggers count accurately every occurrence by using instrumentation or PMCs => probably high overhead.
	...
	\begin{description}[style=nextline]
		\item[What is the difference between profiling and tracing?] 
            Profiling summarises information about the runtime spent per routine/line/block. 
            Tracing collects call-paths with timing information and is therefore much more detailed.

	\end{description}
	\item[What can hardware performance counter measure?]
        PMCs are MSRs/hw-counters to measure processor events like cache/branch-misses, loads/stores and much more.
	...
	\begin{description}[style=nextline]
		...
		\item[How to avoid expensive operations?] Use pre-calculated tables for fast lookup of results.
		...
		\item[How can temporary variables increase performance?] They can be kept in registers and
			therefore reduce the memory transactions.
		...
		\item[What are problems with C++ codes?] Avoid frequent (de)allocation,
			use static variables whenever possible, avoid using to much abstraction.
...
\begin{description}[style=nextline]
	\item[What can be modelled with the balance metric?] 
        Machine balance: Ratio of memory bandwidth \[Words/s\] to peak
        performance \[Flops/s\].\\
        Code balance: data transfers per arithmetic operations
        $\Longrightarrow$ requirement of the code. \\
        Lightspeed: expected fraction of peak performance = ratio of machine
        balance to code balance.  

	\begin{description}[style=nextline]
		\item[How are lightspeed, machine and code balance defined?] See formulas.
	...
	\begin{description}[style=nextline]
		\item[Which algorithm types have potential for optimizations?] Memory-bound usually not so much.
			Take care for compute-bound problems: \( \frac{O(N^3)}{O(N^2)} \).
	...
	\begin{description}[style=nextline]
		...
		\item[How is the sparse matrix-vector multiply computed and optimized?] Working set shrinking by
